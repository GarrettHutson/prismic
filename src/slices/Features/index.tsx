...
const components: JSXMapSerializer = {
  ...
  heading3: ({ children }) => (
    <Heading
      as="h3"
      size="sm"
      className=" mb-3 font-medium p-2 sm:text-left text-center"
    >
      {children}
    </Heading>
  ),
  ...
  paragraph: ({ children }) => (
    <Paragraph className=" text-base font-medium font-body text-slate-500 sm:text-left text-center">
      {children}
    </Paragraph>
  ),
};
...
const Features = ({ slice }: FeaturesProps): JSX.Element => {
  return (
    ...
    >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 place-items-center sm:place-items-start mx-auto gap-y-12 gap-x-8 max-w-5xl">
